...
+ (NSBezierPath*)bezierPathWithRoundRectInRectWithProgress:(NSRect)aRect radius:(float)radius percentage:(int)percentage
{
	...
	radius = MIN(radius, 0.5f * MIN(NSWidth(aRect), NSHeight(aRect)));
	NSRect rect = NSInsetRect(aRect, radius, radius);
	float progressWidth = (NSWidth(aRect)/100)*percentage;
	
	if((progressWidth>radius) && (progressWidth<NSWidth(rect)+radius))
	{
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMinY(rect)) 
										 radius:radius startAngle:180.0 endAngle:270.0];		
	
		[path lineToPoint:NSMakePoint(NSMinX(rect)+progressWidth-radius, NSMinY(rect)-radius)];
		[path lineToPoint:NSMakePoint(NSMinX(rect)+progressWidth-radius, NSMinY(rect)+radius)];
	...
	else if(progressWidth<=radius)
	{
		...
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMinY(rect)) 
										 radius:radius startAngle:180.0 endAngle:180.0 + (90.0 * (progressWidth/radius)) ];		
		
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMaxY(rect)) 
										 radius:radius startAngle: 180.0 -	(90.0 * (progressWidth/radius)) endAngle: 180.0];		
	}
	//else
	else if(progressWidth>=(NSWidth(rect)+radius))
	{
		NSLog(@"progress 3: %f w:%f r:%f" ,progressWidth, NSWidth(rect), radius);
		
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMinY(rect)) 
										 radius:radius startAngle:180.0 endAngle:270.0];
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMaxX(rect), NSMinY(rect)) 
										 radius:radius startAngle:270.0 endAngle:270.0 + (90.0 * ((NSWidth(rect)+radius-progressWidth)/radius))];
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMaxX(rect), NSMaxY(rect)) 
										 radius:radius startAngle:  90.0 -	(90.0 * ((NSWidth(rect)+radius-progressWidth)/radius))  endAngle: 90.0];
		[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMaxY(rect)) 
										 radius:radius startAngle: 90.0 endAngle:180.0];	
	}
